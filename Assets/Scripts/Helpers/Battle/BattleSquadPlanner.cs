...
namespace OnlyWar.Scripts.Helpers.Battle
{
    public class BattleSquadPlanner
    {
        ...
        private float EstimateHitDistance(ISoldier soldier, RangedWeapon weapon, float targetSize, int freeHands)
        {
            ...
            baseTotal += BattleMathHelpers.CalculateRateOfFireModifier(weapon.Template.RateOfFire);
            baseTotal += BattleMathHelpers.CalculateSizeModifier(targetSize);
            ...
            return BattleMathHelpers.GetRangeForModifier(10.5f - baseTotal);
        }
        ...
        {
            float sizeMod = BattleMathHelpers.CalculateSizeModifier(target.Soldier.Size);
            ...
            float rangeMod = BattleMathHelpers.CalculateRangeModifier(range, target.CurrentSpeed);
            float rofMod = BattleMathHelpers.CalculateRateOfFireModifier(weapon.Template.RateOfFire);
        ...
        {
            float effectiveStrength = BattleMathHelpers.CalculateDamageAtRange(weapon, range);
